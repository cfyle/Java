	...
	public static void main(String[] args) {
		...
		while (end) {
			...
			if (choose < 1 || choose > 3) // 1~3을 벗어날 시
				...
				if (money > 0) { // 음수 입력 판단
					while ((change + money) < menuPrice) { // 모자란 금액을 확인하여 추가 금액을 요청

						System.out.printf("%d원만큼 모자랍니다. " + "돈을 더 넣어주세요(현재 잔액 : %d원)\n", menuPrice - (change + money),
								change + money);
						System.out.print("금액 입력 : ");
						money += sc.nextInt();
					}

					change += money - menuPrice; // 남은 거스름돈과 합쳐줌
					...
					if (checkEnd < 1 || checkEnd > 2) { // 반복문 탈출 조건문
						...
						if (change != 0)// 거스름돈이 0원이 아닐 때
							System.out.printf("거스름돈 %d원을 드리겠습니다.\n", change);
						System.out.printf("감사합니다. 안녕히 가세요.\n");
						end = false;
					}
				} else {
					System.out.printf("잘못된 숫자를 입력했습니다.\n");
					System.out.printf("거래를 종료합니다.\n");
					end = false;
				}
